...
from fantaso.bp_shop.models import Product, ProductImage, Order
...
bp_admin_ext = Blueprint(
        'bp_admin_ext',
        __name__,
)

_bp_admin_ext = Admin(app, template_mode = 'bootstrap3')
...
_bp_admin_ext.add_view(PkView(Product, db.session))
_bp_admin_ext.add_view(PkView(ProductImage, db.session))
_bp_admin_ext.add_view(PkView(Order, db.session))

# def get_path():
#     return join(dirname(__file__)[:-13], 'static/products')
# _bp_admin_ext.add_view(FileAdmin(get_path(), '/products/', name='ProductImage'))
